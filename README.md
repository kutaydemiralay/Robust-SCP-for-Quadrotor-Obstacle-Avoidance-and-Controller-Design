...
We will incorporate methods such as MPC, LQR, Simulink-based simulations, Stateflow, PID control, WARC, convex halfplanes, and funnel synthesis to robustify SCP against wind disturbances, which the SCP framework does not naturally account for.
...
#### Stateflow Chart for Adaptive PID Gains

I integrated a **Stateflow chart** to dynamically adjust PID gains based on the quadrotor’s operating conditions. 
The chart switches between two modes:  

- **Mode 1:** Normal PID gains for nominal wind conditions.  
- **Mode 2:** Aggressive PID gains for enhanced disturbance rejection.  

The mode switching is triggered using two thresholds:  

1. **Position error threshold** – Euclidean distance from the reference trajectory:
   $$
   e_{pos} = \sqrt{(x-x_{ref})^2 + (y-y_{ref})^2 + (z-z_{ref})^2}
   $$
2. **Minimum obstacle distance threshold** – Distance to the nearest obstacle.
...
![Simulink StateFlow Block ](./images/Stateflow.png)

**Figure 11:** . Simulink StateFlow Block 



![Quadcopter Trajectory Optimization](images/Trajoptgif.gif)

**Figure 12:** . Simulink Controller 3D Animation GIF (Same Scenario as Figure 1)



![Quadcopter Trajectory Optimization](images/ez.gif)

**Figure 13:** . Simulink Controller 3D Animation GIF: No Wind-Resisting PID Controller in Lateral Motion (Same Scenario as Figure 2)
